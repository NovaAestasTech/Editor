...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action.includes("prompt")) {
      ...
      systemPrompt +=
        "Expand and elaborate on the given text while maintaining its style and meaning. ";
    } else if (action.includes("shorten")) {
      systemPrompt +=
        "Make the text more concise while preserving key information. ";
    } else if (action.includes("fix")) {
      systemPrompt +=
        "Fix any spelling and grammar errors in the text. Keep the same meaning and style. ";
    ...
    if (format === "rich-text") {
      systemPrompt +=
        "You may use basic HTML formatting like <strong>, <em>, <p>, <ul>, <li>, <ol> when appropriate. ";
    } else {
      systemPrompt +=
        ...
        async start(controller) {
          try {
            ...
            try {
              ...
              const textMatches = fullResponse.match(
                /"text"\s*:\s*"([^"\\]*(\\.[^"\\]*)*)"/g,
              );
              if (textMatches) {
